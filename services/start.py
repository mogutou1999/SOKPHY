...
async def get_site_stats(db: AsyncSession) -> SiteStats:
    try:
        ...
        total_revenue_q = select(
            func.coalesce(func.sum(Order.total_amount), 0.0)
        ).where(Order.status.in_(["paid", "shipped"]))
        shipped_orders_q = select(func.count(Order.id)).where(Order.status == "shipped")
        refunded_orders_q = select(func.count(Order.id)).where(
...
async def handle_stats(message: Message):
    ...
    async with async_session_maker() as db:
        ...
        if not is_admin(user_id):
            await message.answer("🚫 无权限查看统计数据")
            return
        ...
        try:
            ...
            await message.answer("❌ 无法加载统计数据，请稍后再试")
